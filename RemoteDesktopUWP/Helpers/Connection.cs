...
{
	private static readonly XmlSerializer Serializer = new(typeof(Connection));
	...
	public static Connection Parse(string path)
	{
		...
		return Serializer.Deserialize(stream) as Connection ?? throw new InvalidOperationException($"Failed to deserialize {path}");
	}
